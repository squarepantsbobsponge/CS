    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if len(results)==1 and user_role == 'student':
                ...
                session['status']=results[0][3] #些许改动，因为数据库返回不一样，所以需要调整
...
def book_seat():
    ...
    if form.validate_on_submit():
        ...
        try: 
            ...
            if current_time>=time(start_time+7,0):
                flash(f"预约时间不得晚于当前时间，请重试")
                return render_template('book_seat.html', form=form)  
...
def admin_page():
    if 'userid' not in session:
        flash('you must be restart to log in admin_page!','erroe')
        return redirect(url_for('login'))
...
def signin():
    ...
    if form.validate_on_submit():
        ...
        if results :
            ...
            pursed_time = datetime.strptime(sign_time,"%Y-%m-%d %H:%M:%S")
            #判断签到时间
            # 签到成功
        ...
        return render_template('signin.html',form=form,results=result)
    #输入是id sid和签到时间
    ...
    if form.validate_on_submit():
        ...
        if int(choice) == 1:
            ...
            if sid == '':#查询所有黑名单记录
                ...
                query = sql.SQL("Select * from students where status != 0 and sid=%s")
                params = (sid,)
            cursor.execute(query,params)
            results = cursor.fetchall()
            print(results)
            ...
            if sid == '':#学号没有无法添加/修改
                # flash('修改或者添加时学号不能为空，发生错误，请稍后再试。', 'error')
                result = (choice,None)
            else:
                if status == '':
                    # flash('修改或者添加时冷冻时间不能为空，发生错误，请稍后再试。', 'error')
                    result = (choice,None)
                else:
                    ...
                    cursor.execute(query1,params1)
                    cnn.commit()
                    query2 = sql.SQL("Select * from students where sid=%s")
                    params2 = (sid,)
                    cursor.execute(query2,params2)
                    results=cursor.fetchall()
                    result = (choice,results)#返回choice和处理结果
        cnn.commit()
        cursor.close()
        cnn.close()
...
if __name__=='__main__':
    ...
    schedule.every().day.at("18:02").do(everyday_update)
