...
class LR:
    ...
    def Hypothesis_Function(self,index:int):#计算多层感知机的的假设值，index是要计算的数据的标号
        #直接运算,第一个参数0次的，偏置因子不要乘特征  单层感知机是sign函数不是sigmod函数
    ...
    def showgraph(self):##最终答案
             ...
             x1_values = np.linspace(0, 1,30)  # 替换起始值、结束值和点数为你需要的值  ##随机生成30个值给x1
             print("参数为：",self.arg)
             x2_values = (0- self.arg[0] - self.arg[1]*x1_values) / self.arg[2] #根据生成的x1按照模型计算x2的值 
