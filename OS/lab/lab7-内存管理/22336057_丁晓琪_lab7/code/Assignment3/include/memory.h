...
public:
    ...
    void initialize();
    //初始化内核空闲池
    void init_FIFO(int start_address);
    //置换:参数为需要的帧数
    void displace_page(int count,int vaddr);

    void releasePages_FIFO(enum AddressPoolType type, const int virtualAddress, const int count); 
    // 从type类型的物理地址池中分配count个连续的页
    // 成功，返回起始地址；失败，返回0
    int allocatePhysicalPages(enum AddressPoolType type, const int count,int vaddr);
