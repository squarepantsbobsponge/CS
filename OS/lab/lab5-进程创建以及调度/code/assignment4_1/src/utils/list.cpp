...
{
    head.next = head.previous = 0;
    head.priority=0;//初始化为0，到初始化线程时候加上
}
...
//插入优先级对应位置
void List::insert_priority(ListItem *itemPtr){
   //list带头节点的双向列表//头节点的优先级默认为0了，最低，可以放心从第一个节点开始插入
   ListItem *temp = head.next;
   int flag=0;//判断是否插入
   if(size()==0) {
    head.next=itemPtr;
    ...
    while(temp){
        if(itemPtr->priority>=temp->previous->priority&&itemPtr->priority<=temp->priority){
             itemPtr->previous = temp->previous;
            itemPtr->next = temp;
            temp->previous->next = itemPtr;
            temp->previous = itemPtr;
