...
int STDIO::print(const char *const str)//实现换行
{
    int i = 0;
    ...
    for (int i = 0; fmt[i]; ++i)
    {
        ...
        else
        {
            ...
            case 'c':
                ...
                break;//按照char格式提取可变参数，加到缓冲区

            case 's':
                buffer[idx] = '\0';
                idx = 0;
                counter += stdio.print(buffer);//先清空打印缓冲区
                counter += stdio.print(va_arg(ap, const char *));//直接打印字符串
                break;

            case 'p':
                temp0 = va_arg(ap, int);
                itos(number, temp0, 16);//转为字符串
            ...
            case 'x':
                ...
                if (temp < 0 && fmt[i] == 'd')
                {
                    ...
                    temp = -temp;//计数器counter
                }

                itos(number, temp, (fmt[i] == 'd' ? 10 : 16));//转为字符串
