...
void four_thread(void *arg) {
             ...
             printf("pid %d name \"%s\": Hello World!\n", programManager.running->pid, programManager.running->name);
                ...
                while (delay)
                     --delay;
             //if(programManager.running->priority==2){//优先级转为2了
                printf("pid %d name \"%s\": Hello World!\n", programManager.running->pid, programManager.running->name);//打印一次就退出
            //}
...
void first_thread(void *arg)
{
    ...
    {//放开这里调度
        ...
        programManager.executeThread(third_thread, nullptr, "third thread", 1);
                 programManager.executeThread(four_thread, nullptr, "four thread", 1);
    }
   //asm_halt();
}
