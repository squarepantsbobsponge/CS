...
void STDIO::moveCursor(uint position)//移动光标位置
{
    if (position >= 80 * 25)
    {
        return;
    ...
    asm_out_port(0x3d4, 0x0e);//表示处理高8位
    asm_out_port(0x3d5, temp);//表示要处理的高8位的数据是什么

    // 处理低8位
    temp = position & 0xff;
    asm_out_port(0x3d4, 0x0f);//低8位
...
void STDIO::moveCursor(uint x, uint y)//移动光标二维版
{
    if (x >= 25 || y >= 80)
    {
        return;
...
void STDIO::rollUp()//滚屏函数，每一行都上一一行
{
    uint length;
    length = 25 * 80;
