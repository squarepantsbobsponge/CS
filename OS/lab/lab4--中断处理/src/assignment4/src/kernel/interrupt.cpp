...
void InterruptManager::enableTimeInterrupt()
{
    ...
    // 开启主片时钟中断，置0开启// 时钟中断是IRQ0，把0位的中断屏蔽去除
    value = value & 0xfe;
...
extern "C" void c_time_interrupt_handler()
{
    ...
    char str[] = "22336057!_My_name_is_dingdong!";
    char number[10];
    int temp = times;
    ...
    // 移动光标到(0,0)输出字符//每次跑五个亮其他颜色//亮灯开头位置
    
    if(a>=30) a=0;//光标到边界了回到开头
    stdio.moveCursor(0);
    for(int i = 0; str[i]; ++i ) {
        if(i>=a&&i<(a+5)){//a跑马灯计数器
           stdio.print(str[i],0x0e);
        }
        else{
            stdio.print(str[i]);
        }
